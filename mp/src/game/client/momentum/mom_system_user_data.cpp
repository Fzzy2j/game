...
bool UserData::ParseFromKV(KeyValues *pUserKv)
{
    ...
    m_uID = pUserKv->GetUint64("steamID");
...
void UserData::SaveToKV(KeyValues *pKvOut)
{
    pKvOut->SetString("alias", m_szAlias);
    pKvOut->SetUint64("steamID", m_uID);
...
{
    if (FStrEq(event->GetName(), "site_auth"))
    {
    ...
    else if (FStrEq(event->GetName(), "run_upload"))
    {
        if (!event->GetBool("run_posted"))
            return;

        m_LocalUserData.m_iCurrentLevel += event->GetInt("lvl_gain");
        m_LocalUserData.m_iCurrentXP += event->GetInt("cos_xp");
...
void MomentumUserData::FireUserDataUpdate()
{
    ...
    {
        vgui::ivgui()->PostMessage(m_vecUserDataChangeListeners[i], new KeyValues("UserDataUpdate"), null);
    }
