...
public:
    ...
    {
        const auto pOwnerName = SteamFriends()->GetFriendPersonaName(hAvatarID);
        
        return pOwnerName[0] ? CFmtStr("%s's Lobby", pOwnerName).Get() : "<A public Lobby>";
    }
...
public:
    ...
    const char *GetLobbyName(const CSteamID &hLobbyID, uint64 hAvatarID) override
    {
        const auto pFriendName = SteamFriends()->GetFriendPersonaName(hAvatarID);
        if (pFriendName[0])
        {
            const auto index = m_mapLobbyIDToFriendsID.Find(hLobbyID.ConvertToUint64());
            if (m_mapLobbyIDToFriendsID.IsValidIndex(index))
            {
                ...
                if (iCount > 1)
                {
                    return CFmtStr("%s (+ %i)", pFriendName, iCount - 1);
                }
            }
...
LobbySearchPanel::LobbySearchPanel(Panel *pParent, bool bSearchingFriends) : BaseClass(pParent, "LobbySearchPanel")
{
    ...
    AddColumnHeader(1, "name", "Name", GetScaledVal(100), GetScaledVal(60), GetScaledVal(160), COLUMN_RESIZEWITHWINDOW);
    AddColumnHeader(2, "map", "Map", GetScaledVal(100));
    AddColumnHeader(3, "slots", "Players", GetScaledVal(20));
