...
class LobbyChatContainer : public Panel
{
public:
    DECLARE_CLASS_SIMPLE(LobbyChatContainer, Panel);
    LobbyChatContainer(Panel *pParent) : BaseClass(pParent, "LobbyChat")
    {
    ...
    void OnThink() override
    {
        if (g_pChatPanel->GetParent() != this)
        {
            ...
            g_pChatPanel->StartMessageMode(MESSAGE_MODE_MENU);
...
{
    m_pLobbyInfo->OnLobbyChatUpdate(pParam);
...
void DrawerPanel_Lobby::OnResetData()
{
    ...
    m_pLobbyChat = new LobbyChatContainer(this);
